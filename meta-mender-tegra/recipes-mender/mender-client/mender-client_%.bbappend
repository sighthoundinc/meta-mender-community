...
SRC_URI_append = " \
    ...
    file://verify-boot-rootfs-slot-alignment-cboot \
    file://verify-boot-rootfs-slot-alignment-no-redundant-bootloader \
"

FILES_${PN} += " \
                    ${systemd_system_unitdir}/mender-client-commit.service \
                    ${sysconfdir}/mender/mender-client-commit.sh \
                    ${bindir}/mender_base \
                    ${sysconfdir}/mender/verify-boot-rootfs-slot-alignment \
"
...
install_slot_align_verify() {
    install -m 0755 ${WORKDIR}/verify-boot-rootfs-slot-alignment-cboot ${D}${sysconfdir}/mender/verify-boot-rootfs-slot-alignment
}
...
install_slot_align_verify_mender-uboot() {
    install -m 0755 ${WORKDIR}/verify-boot-rootfs-slot-alignment-uboot ${D}${sysconfdir}/mender/verify-boot-rootfs-slot-alignment
}
...
install_slot_align_verify_no_redundant_bootloader() {
    install -m 0755 ${WORKDIR}/verify-boot-rootfs-slot-alignment-no-redundant-bootloader ${D}${sysconfdir}/mender/verify-boot-rootfs-slot-alignment
}
